// Generated on 2021-05-20 using generator-mendix 2.2.5 :: git+https://github.com/mendix/generator-mendix.git
/* eslint-disable */
"use strict";
...
module.exports = function (grunt) {
    grunt.initConfig({
        ...
        copy: {
            deployment: {
                files: [{ dest: TEST_WIDGETS_DEPLOYMENT_FOLDER, cwd: "./src/", src: ["**/*"], expand: true }]
            },
            mpks: {
                files: [{ dest: TEST_WIDGETS_FOLDER, cwd: "./dist/", src: [pkg.widgetName + ".mpk"], expand: true }]
            }
        },
        clean: {
            build: [path.join(currentFolder, "dist", pkg.widgetName, "/*")]
        },
        csslint: {
            strict: {
                options: {
    ...
    grunt.registerTask("start-modeler", function () {
        ...
        if (
            MODELER_PATH !== null ||
            (mendixApp.err === null && mendixApp.output !== null && mendixApp.output.cmd && mendixApp.output.arg)
        ) {
            grunt.util.spawn(
                {
                    cmd: MODELER_PATH || mendixApp.output.cmd,
                    args: [(MODELER_PATH !== null ? MODELER_ARGS : mendixApp.output.arg).replace("{path}", TEST_PATH)]
                },
                function () {
                    done();
                }
            ...
            if (res.package.clientModule[0].$.version) {
                ...
                if (!version) {
                    ...
                    if (!semver.valid(version) || !semver.satisfies(version, ">= 1.0.0")) {
                        grunt.log.error(
                            "\nPlease provide a valid version that is higher than 1.0.0. Current version: " +
                            currentVersion
                        );
                        done();
                    } else {
    ...
    grunt.registerTask("clean build", "Compiles all the assets and copies the files to the build directory.", [
        "clean",
        "compress",
        "copy"
    ]);
